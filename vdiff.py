...
def get_differing_value_ranges(
    code: str,
) -> tuple[str, str, list[tuple[str, Range, Range]]]:
    # return [code, instrumented_code, list[tag,gcc_11_range,gcc_12_range]]
    instrumented_code = instrument(code)
    return (
        code,
        instrumented_code,
        get_differing_value_ranges_from_instrumented(instrumented_code),
    )


def preprocess_and_get_differing_value_ranges(
    code: str,
) -> tuple[str, str, list[tuple[str, Range, Range]]]:
    # return [code, instrumented_code, list[tag,gcc_11_range,gcc_12_range]]
    code = preprocess_csmith_code(code, "gcc", ["-isystem/usr/include/csmith-2.3.0"])
    instrumented_code = instrument(code)
    return (
        code,
        instrumented_code,
...
def reduction_check(code: str) -> bool:
    return len(get_differing_value_ranges(code)[2]) > 0
...
gcc11 = "/home/theo/.cache/ccbuilder-compilers/gcc-01a9836c07053b3286b2259d6b628c7583413f55/bin/gcc"
gcc12 = "gcc"
...
if __name__ == "__main__":
    ...
    short a, b;
    int c;
    unsigned short(d)(unsigned e) { return e - a; }
    void f() {
      int g = 0;
      for (;;) {
        c = d(b);
    ...
    int main() {}
    """
    print(get_differing_value_ranges(case_)[1])
    exit(0)
    ...
    if False:
        with ProcessPoolExecutor(cpu_count()) as p:
            ...
            for case_ in CSmithGenerator().generate_code_parallel(100, p):
                futures.append(p.submit(preprocess_and_get_differing_value_ranges, case_))
    ...
    print(get_differing_value_ranges(reduced)[2])
