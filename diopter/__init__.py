...
from diopter.builder import Builder, BuilderWithoutCache
from diopter.defaults import (
    DEFAULT_PATCH_DIR,
    DEFAULT_PREFIX_PARENT_PATH,
    DEFAULT_REPOS_DIR,
)
from diopter.patchdatabase import PatchDB
from diopter.patcher import Patcher
...
from diopter.utils import get_compiler_info, initialize_patches_dir, initialize_repos
...
def ccbuilder_base_parser() -> ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=f"Path to the directory with the compiler repositories \
    ...
    parser.add_argument(
        "--patches",
        nargs="+",
        help="For the 'patcher', this defines which patch(es) to apply.\
                 For the 'builder', these are the additional patches to \
                 apply to the already existing patches from the PatchDB.",
        type=str,
    )

    parser.add_argument(
        "--logdir",
        help="Path to the directory where log files should be stored in.\
                If not specified, ccbuilder will print to stdout.",
        type=str,
    )
