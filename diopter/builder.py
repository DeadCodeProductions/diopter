...
from diopter.repository import Commit, Repo, Revision, select_repo
from diopter.utils import CompilerProject, run_cmd_to_logfile
...
def patch_if_necessary(
    ...
    Args:
        ...
        working_dir (Path):
            Path to git repository which has `commit_to_patch` checked out.
        additional_patches (list[Path]):
            Additional patches to apply *after* the ones found in `patchdb`.
...
def llvm_build_and_install(
    ...
    Args:
        ...
        configure_flags (str | None):
            additional flags to pass to the configure script or cmake.
...
def build_and_install_compiler(
    ...
    Args:
        ...
        get_executable (bool):
            Whether or not to return the path to the executable compiler instead.
    ...
    Returns:
        Path:
            `cache_prefix`/projectname-commit or
            `cache_prefix`/projectname-commit/bin/$compiler if `get_executable` is set.
    """
...
def _worker_alive(worker_indicator: Path) -> bool:
    if worker_indicator.exists():
        ...
        try:
            ...
            # from man 1 kill
            # > If signal is 0, then no actual signal is sent,
...
class Builder(BuilderWithoutCache):
    def build(
        ...
        """Build the specified compiler and return the path to the
        installation location. If the specified compiler has already been built,
        `build` will return the cached path.
        ...
        Args:
            ...
            configure_flags (str | None):
                additional flags to pass to the configure script or cmake.
            force (bool):
                Build the specified compiler even if it has been cached before.

        Returns:
            Path:
                `cache_prefix`/projectname-commit or
                `cache_prefix`/projectname-commit/bin/$compiler if
                `get_executable` is set.
        """
        ...
        if not force:
            if success_indicator.exists():
                ...
                logging.info(
                    "This compiler seems to be built by some other worker. \
                            Need to wait. If there is no other worker, \
                            abort this command and run ccbuilder cache clean."
                )
                ...
                while (
                    ...
                    if time.time() - start_time > 15 * 60:
                        counter += 1
                        logging.info(
                            f"{counter*15} minutes have passed waiting. \
                                    Maybe the cache is in an inconsistent state."
                        )
                        start_time = time.time()
