...
To achieve this, it heavily uses git and bisections and with that the terms 'good' and
'bad'.However, 'good' does mean something different in different circumstances does not
always indicate a successful build.

The convention used for good and bad is taken from the normal use-case of `git bisect`:
    Something broke along the way and is now bad. Additionally, we know a commit,
    which worked, which is good.
...
from diopter.builder import Builder, BuildException
from diopter.patchdatabase import PatchDB
from diopter.repository import Commit, Repo, Revision, get_releases
from diopter.utils import CompilerProject
...
class Patcher:
    ...
    def _check_building_patches(
        ...
        if not self.patchdb.requires_all_these_patches(
            ...
            try:
                ...
                logging.critical(
                    f"Failed to build {rev} with patches {patches}. \
                            Manual intervention needed. Exception: {e}"
                )
    ...
    def _adjust_bisection_midpoint_after_failure(
        ...
        """
        Move the midpoint either forwards or backwards (depending on the
        double_fail_counter)
    ...
    def _find_oldest_ancestor_not_needing_patches_and_oldest_patchable_from_releases(
        ...
        Returns:
            tuple[Optional[Commit], Optional[Commit]]: Common ancestor (w.r.t. releases)
                that needs no patch and the oldest patchable commit found.
        """
        ...
        for old_version in release_versions:
            ...
            if not repo.is_branch_point_ancestor_wrt_master(
                ...
                if oldest_patchable_ancestor:
                    ...
                    logging.warning(
                        "Found only older releases requiring the patch(es)!"
                    )
                    break
                raise Exception(
            ...
            if common_ancestor in tested_ancestors:
                logging.info(
                    f"Common ancestor of {old_version} and \
                            {potentially_human_readable_name} was already tested.\
                            Proceeding..."
                )
                continue
    ...
    def find_ranges(
        ...
        Args:
            project (CompilerProject): The compiler project which has the
                problematic commit.
            repo (Repo): Repository corresponding to `project`.
            patchable_commit (Commit): The problematic commit which is buildable
                with `patches`.
        ...
        ) = self._find_oldest_ancestor_not_needing_patches_and_oldest_patchable_from_releases(  # noqa: E501
            project,
            repo,
            patchable_commit,
        ...
        #   - This only happens if a bug was re-introduced or the patch just so happens
        #     to fix another intermediate bug.
        ...
        #   - Find fixer commits from there
        #     (could do something if no_patch_common_ancestor exists)
    ...
    def find_fixer_from_introducer_to_releases(
        ...
        for release in reachable_releases:
            ...
            # As we assume that a fixer at a given point fixes all its children,
            # this is fine.
            logging.info("Checking for known fixers...")
            if (
                ...
                logging.info("Already known fixer. No additional searching required")
                continue
            ...
            if patching_result is PatchingResult.BuildFailed:
                ...
                # release only builds with patch, everything to release is
                # to be included
    ...
    def bisect_build(
        ...
        Args:
            ...
            bad (Commit): Commit that is further away from the head of the branch
                than `good`.
            project (CompilerConfig): Project being patched.
            repo (Repo): Repository for `project`.
            failure_is_good (bool): If failing to build is expected to happen to the
                `good` commit.
    ...
    def find_introducer(
        ...
        while True:
            ...
            try:
                ...
                exp += 1
                logging.info(
                    f"Failed to build {current_commit}. \
                            Increasing exponent to {exp}: {e}"
                )

        # ====== Bisection
        msg = f"Staring bisection between {current_commit} and {prev_commit}, \
                should take at most around \
                {math.log(max(2**exp, 11) - 2**min(exp-1, 0), 2)} steps"
        logging.info(msg)
        print(msg)
