...
import diopter.utils as utils
from diopter.defaults import DEFAULT_REPOS_DIR
...
class Repo:
    ...
    def get_best_common_ancestor(self, rev_a: Revision, rev_b: Revision) -> str:
        ...
        return utils.run_cmd(f"git -C {self.path} merge-base {a} {b}").stdout
    ...
    def rev_to_commit(self, rev: Revision) -> Commit:
        ...
        try:
            ...
            return utils.run_cmd(f"git -C {self.path} rev-parse {rev}").stdout
    ...
    def rev_to_range_needing_patch(
        ...
        if len(merges_after_introducer) > 0:
            ...
            try:
                merger_parents = set(utils.run_cmd(cmd).stdout.split("\n"))
            except subprocess.CalledProcessError as e:
                raise RepositoryException(e)
        ...
        try:
            res = [
                commit
                for commit in utils.run_cmd(cmd).stdout.split("\n")
                if commit != ""
            ] + [introducer]
            return res
        except subprocess.CalledProcessError as e:
    ...
    def get_unix_timestamp(self, rev: Revision) -> int:
        ...
        try:
            return int(
                utils.run_cmd(f"git -C {self.path} log -1 --format=%at {rev}").stdout
            )
        except subprocess.CalledProcessError as e:
            raise RepositoryException(e)
    ...
    def next_bisection_commit(self, good: Revision, bad: Revision) -> Commit:
        ...
        try:
            return utils.run_cmd(request_str).stdout
        except subprocess.CalledProcessError as e:
            raise RepositoryException(e)
...
def select_repo(
    project: utils.CompilerProject, llvm_repo: Repo, gcc_repo: Repo
) -> Repo:
    match project:
        case utils.CompilerProject.LLVM:
            repo = llvm_repo
